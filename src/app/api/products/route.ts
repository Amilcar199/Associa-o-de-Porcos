...
export async function POST(req: NextRequest) {
  try {
    ...
    // 1) Tentar vincular ao usuário da sessão (Mongo) pelo email
    let seller: { id: string } | null = null
    const sessionUser = (authResult as any).user
    const sessionEmail = sessionUser?.email ? String(sessionUser.email).toLowerCase().trim() : null
    ...
    try {
      if (sessionEmail) {
        ...
        if (existingByEmail) {
          // Garantir que está ativo e com role admin
          if (existingByEmail.role !== 'admin' || existingByEmail.isActive !== true) {
            ...
            seller = { id: existingByEmail.id }
          }
        } else {
          // Criar o usuário admin correspondente no Prisma (senha não utilizada pelo login)
      ...
      console.error('Falha ao sincronizar usuário admin no Prisma:', e)
    }

    // 2) Fallback: procurar qualquer admin ativo no Prisma
    if (!seller) {
      seller = await prisma.user.findFirst({ where: { role: 'admin', isActive: true }, select: { id: true } })
    }

    // 3) Último recurso (ambiente de desenvolvimento): criar um admin demo
    ...
    if (!seller) {
      return errorResponse('Nenhum administrador ativo encontrado para atribuir como vendedor. Crie um admin em Usuários (MySQL) e tente novamente.')
    }

    // Mapear campos do formulário para o schema Product
    // Normalizar tipos onde necessário
    const ageInt = Math.round(Number(sanitizedData.age))
    const weightInt = Math.round(Number(sanitizedData.weight))
    if (!Number.isFinite(ageInt) || ageInt < 0) return errorResponse('Idade inválida')
    if (!Number.isFinite(weightInt) || weightInt <= 0) return errorResponse('Peso inválido')
